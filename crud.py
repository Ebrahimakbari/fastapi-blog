...
class UserCrud:
    ...
    async def get_user_by_email(db: AsyncSession, email:EmailStr) -> Optional[User]:
        user = await db.execute(select(User).where(User.email == email).options(selectinload(User.posts)))
        return user.scalar_one_or_none()

    @staticmethod
    async def get_user_by_id(db: AsyncSession, id: int) -> Optional[User]:
        user = await db.execute(select(User).where(User.id == id).options(selectinload(User.posts)))
        return user.scalar_one_or_none()
...
class PostCrud:
    ...
    async def get_posts(db: AsyncSession, skip:int = 0, limit:int = 10) -> list[Post]:
        posts = await db.execute(
            select(Post).options(selectinload(Post.owner)).offset(skip).limit(limit).order_by(Post.created_at.desc())
        )
        return posts.scalars().all()
    
    @staticmethod
    async def get_post(db: AsyncSession, post_id:int):
        post = await db.execute(select(Post).where(Post.id == post_id).options(selectinload(Post.owner)))
